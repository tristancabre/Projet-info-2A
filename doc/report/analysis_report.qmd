...
This distinction is needed because some services and mandatory features require internal access to information that ordinary users should not have access to, such as the connection history of other users, system logs, or usage statistics across all accounts. Allowing every user unrestricted access to this kind of data would compromise both the privacy of other users and the overall security of the application. 

This is why an authentication controller is required. It must identify the user upon login and grant access to the appropriate personal interface and set of tools depending on their role. In practice, Administrators can be seen as regular visitors with extended rights, allowing them to manage the application (for example, checking user activity or auditing access to specific features), rather than as a completely separate category of user. 

We decided to build the user interface with Streamlit. Instead of having Streamlit communicate directly with the database, we have to use FastAPI a backend API layer between the frontend and the data. Then, the path of a request will be as such : the Streamlit frontend calls the API, which forwards the request to a Controller, the Controller delegates the business logic to a Service layer which relies on a DAO (Data Access Object) to communicate with the database. This separation of concerns makes the application easier to maintain, test, and extend, since each layer has a single, well-defined responsibility. 

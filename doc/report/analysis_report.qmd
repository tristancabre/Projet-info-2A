...
## Introduction {.unnumbered}
  ...
  Who has never watched shooting stars in the sky as a kid, wondering what they were and where they came from ? Growing up, we all learned that these beautiful lights were due to the high-speed dust of meteorit coming in the atmosphere, before it inevitably destroys it. However, what would happen if it was not just the dust of the meteorit, but the meteorit itself coming straight to us through the atmosphere ? Wouldn't we like to know ? And if we do, could we avoid it ? At least, we know that the dinosaurs didn't ask themselves this question…
  ...
  The objective of this project is to develop an intuitive and secured application, coded in Python, that queries NASA's API in order to track and monitor these Near-Earth Objects. We decided to name our application K.L.S.W for Ker Lann Space Watch. Designed to handle distinct access levels for regular users and administrators, the system provides an interactive dashboard highlighting close approaches and hazardous celestial bodies. 
   
  From querying specific NEOs by diameter or trajectory to managing personalized favorites and registering newly spotted objects, the application delivers actionable planetary defense insights at anyone's fingertips. The first part of this report will be devoted to the preliminary analysis of the project, detailing the functional requirements, constraints, and team organization. The second part will address the technical design, including the use case and activity diagrams, as well as the database schema and architecture.
...
Beyond the formal list of functionalities given in the specifications, it is useful to describe how a user actually moves through NEO-Watch, screen by screen, since this is what ultimately determines whether the application feels intuitive while following the features, both necessary and optional. The whole application is organised as a set of menus, each one giving access to a limited number of clearly identified actions, so that a user or an administrator is never more than a few clicks away from the feature they are looking for. 

A first-time user lands on the Welcome screen, where the only choices are to log in, to register, or to leave the application. Registration simply asks for a login, a password and an email address, the latter being required since it is later used to deliver alert notifications. Once authenticated, the user is redirected to one of two dashboards depending on their role, both built on the same logic : a synthetic overview of the next close approaches, the number of potentially hazardous NEOs, the closest object over a chosen period, and, for Visitors, the highlights drawn from their own favorites together with any pending alert notification.

From the visitor (user) dashboard, four main menus are reachable, each one directly implementing a mandatory feature. The menu to browse NEOs opens a search screen where the user filters by name, identifier, diameter or next passage date, sorts the results, and exports them if needed; selecting a result opens a screen with NEOs details, from which the object can be added to favorites or, echoing the "add informations" use case, completed with additional information. The menu to create a NEO offers a short form through which an observed object can be registered directly in the database. The favorites menu lists the user's watchlist, lets a NEO be removed from it, and gives access, for each entry, to a dedicated history screen where the evolution of the object's distance to Earth is plotted and can be exported as a graph. Finally, the alerts menu lets the user consult the active notification rules or define a new one, which will later be evaluated at each login.

The administrator dashboard reuses the same overview but replaces the personal menus with the management tools. The menu to manage accounts lets an administrator browse the list of registered users and delete an account. The logs menu gives access to a chosen user's login history, and the menu to update the database triggers a synchronization with NASA's API while guaranteeing that user-created NEOs are preserved. In every case, whatever screen the user is on, a return path brings them back to their own dashboard, and login out from either dashboard always leads back to the Welcome screen.

![Menu diagram](img/Menu_diagram.png)
...
![Gantt diagram](img/Gantt_diagram.png){width=115%}

### Work organisation

Concerning the practical organisation of our work, we have a discussion group on WhatsApp that lets us exchange ideas and keep each other updated on the progress of our tasks. Beyond these day-to-day exchanges, we also hold regular meetings, both during the sessions scheduled for the project and, when needed, outside of them, to review what has been done, clarify the points that still need to be settled, and decide how to distribute the work that remains.

Furthermore, in order to facilitate the completion of the project, we assigned each member of the group a specific role. Tristan is responsible for managing the writing of the report, Gabrielle is the tech lead, Albane is the group manager, and Noé oversees testing in the application.

A GitHub repository has also been set up: this allows us to work as efficiently as possible as a team. We can work simultaneously on the project and benefit from a simplified way of managing conflicts. This GitHub repository is essential for optimal collaboration, and it is also where we keep our diagrams, our Gantt chart and the report itself, so that every member can consult or modify them.

Finally, this application will be developed in Python, mainly through the VS Code editor made available on ENSAI's Onyxia (SSP Cloud) platform.
...
### Use Case diagram
The use case diagram summarizes, for both the user and administrator identified in the specifications, the actions they are entitled to perform once the features are translated into concrete interactions. The user, once logged in, can exit the application, set an alert, manage their list of favorites (which includes removing a NEO that no longer interests them), and look up a NEO; this last use case is central to the application, since it is itself the entry point to two further actions, adding the object to favorites or, as an optional feature, adding information about it. Creating an account is also available to a non-authenticated user, as a prerequisite to logging in and reaching the rest of the menu. The administrator inherits every use case available to the user, since an administrator account can also browse NEOs, manage favorites or set alerts, and is additionally granted the use case of updating the data, a privileged action from which two further use cases branch out: accessing the login history of users and deleting an account. This inheritance relationship, already visible in the class diagram, is reflected here by the fact that the administrator actor is connected to all the use cases reachable by the Visitor in addition to their own, rather than by a duplicated set of unrelated actions.
...
![Activity diagram](img/Activity_diagramm.png){width=90%}
...
The class Neo represents a Near Earth Object, with its descriptive characteristics: name, weight, diameter, composition, rarity, origin, its closest recorded day, and a distance to Earth. This distance is not a fixed, it evolves over time as the object continues its trajectory in the NASA database, which is precisely why it is built so that there can be a mechanism to keep track of it. 

The NeoDistanceHistory class enables to follow how a NEO's distance to Earth evolves after each update of the database. Each Neo should own exactly one NeoDistanceHistory object. It is a component of the Neo itself, responsible for recording the evolution of the distance to Earth. Internally, it keeps a list of records (a date to reference the distance to Earth) which will give the opportunity for a User to reconstruct its evolution, approach or leave over time. 
...
Every User can define one or more Alert entries, each one relying on a condition to trigger the notification. Then, there are two classes Alert and Notification.
...
More than a technical exercise, this first phase turned out to be a good opportunity for the four of us to figure out how we each work best, and where we can lean on one another going forward. Some of us are more comfortable with the visual and structural side of things, others gravitate naturally toward writing and justifying choices in words; knowing this now means we can hand out the remaining work in a way that plays to everyone's strengths rather than splitting it evenly by default.

It was also, for most of us, a first real encounter with several concepts that only existed as abstract terms before this project: what a Data Access Object actually does in practice, how a web service is meant to talk to a database... None of this was obvious at the start, but having gone through it once means we now have a working method we can rely on for the rest of the project.

Above all, sitting down to think through the application before writing a single line of code gave the whole group a shared and precise mental picture of NEO-Watch, one that everyone can now build from without having to check with the others every step of the way. Whether that picture survives contact with the actual implementation is another question entirely, and it would not be surprising if some of the choices made here turn out to need revisiting; the final report will be the place to take stock of what changed and why.

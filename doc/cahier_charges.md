...
More specifically, this application should include the following features
    ...
    F6 : Create alerts. The user will be able to define alerts, centered on NEOs or not, such as: « Notify me when an asteroid more than 100 meters in diameter passes within 5 million kilometers of Earth ». If the application detects such events, a notification will be sent to the user when they log in.

Optional features

    FO1 : Compare several NEOs. The user will be able to select several asteroids and compare their characteristics (diameter, speed, distance from Earth, passage date, danger level, etc.). This comparison may be presented as a table or an exportable graph.
    ...
    FO3 : Build some statistics. The user will be able to access statistics such as the number of asteroids observed per month, diameter distribution, evolution in the number of approaches, average distance from Earth, proportion of objects classified as potentially dangerous, etc. The statistics must be relevant and justified.
    FO4 : Suggest an interesting NEO. The application will recommend to the user a selection of NEOs that could be particularly interesting to observe, based on explicit criteria of interest, which could, for example, combine diameter, distance, speed, rarity of passage, potentially dangerous nature, etc.
